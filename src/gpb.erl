...
analyse_vsn_1([C | T]) when ?is_digit(C) ->
    analyse_vsn_2(T). % must begin with 0-9
...
analyse_vsn_3([C | T]) when ?is_digit(C) ->
    analyse_vsn_2(T). % 0-9 must follow .

analyse_vsn_4([C | T]) when ?is_digit(C) ->
    analyse_vsn_5(T). % 0-9 must follow -
